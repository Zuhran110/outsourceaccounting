// Import images
import HONE from "../assets/images/serivePgsImg/Hero/HONE.png";
import HTOO from "../assets/images/serivePgsImg/Hero/HTOO.png";
import HTHREE from "../assets/images/serivePgsImg/Hero/HTHREE.png";
import HFOOR from "../assets/images/serivePgsImg/Hero/HFOOR.png";
import HFIVE from "../assets/images/serivePgsImg/Hero/HFIVE.png";
import HSIX from "../assets/images/serivePgsImg/Hero/HSIX.png";
import HSEVEN from "../assets/images/serivePgsImg/Hero/HSEVEN.png";
import HeroBackgroundImg from "../assets/images/serivePgsImg/Hero/HeroBackgroundImg.png";
import addbusiness from "../assets/images/serivePgsImg/Get/addbusiness.png";
import affordableCleanEnergy from "../assets/images/serivePgsImg/Get/affordableCleanEnergy.png";
import arttrack from "../assets/images/serivePgsImg/Get/arttrack.png";
import housecompliance from "../assets/images/serivePgsImg/Get/housecompliance.png";
import parkoutline from "../assets/images/serivePgsImg/Get/parkoutline.png";
import userexpert from "../assets/images/serivePgsImg/Get/userexpert.png";
import tickCircle from "../assets/images/WhyOutsourceAccounting/tickcircle.svg";
import whyOutSourceUperImg from "../assets/images/serivePgsImg/WhyChooseUs/WhyChooseUs.png";
import vatreturns from "../assets/images/serivePgsImg/WhyChooseUs/vatreturns.svg";
import companyformation from "../assets/images/serivePgsImg/WhyChooseUs/companyformation.svg";
import corporatetax from "../assets/images/serivePgsImg/WhyChooseUs/corporatetax.svg";
import selfassesment from "../assets/images/serivePgsImg/WhyChooseUs/selfassesment.svg";
import bookkeeping from "../assets/images/serivePgsImg/WhyChooseUs/bookkeeping.svg";
import payroll from "../assets/images/serivePgsImg/WhyChooseUs/payroll.svg";
import everytime from "@/assets/images/serivePgsImg/howitworks/everytime.png";
import gettingtoknow from "@/assets/images/serivePgsImg/howitworks/gettingtoknow.png";
import prepareaccount from "@/assets/images/serivePgsImg/howitworks/prepareaccount.png";
import reviewexplain from "@/assets/images/serivePgsImg/howitworks/reviewexplain.png";
import AnnualAccounts from "../assets/images/serivePgsImg/AnnualAccounts/AnnualAcounts.svg";
import bookeeping from "../assets/images/serivePgsImg/AnnualAccounts/bookeeping.svg";
import companyfoormation from "../assets/images/serivePgsImg/AnnualAccounts/companyfoormation.svg";
import corporation from "../assets/images/serivePgsImg/AnnualAccounts/corporationtax.svg";
import payrol from "../assets/images/serivePgsImg/AnnualAccounts/payrol.svg";
import selfassessment from "../assets/images/serivePgsImg/AnnualAccounts/selfassessment.svg";
import acc from "@/assets/images/serivePgsImg/AnnualAccounts/acc.svg";
import bk from "@/assets/images/serivePgsImg/AnnualAccounts/bk.svg";
import cf from "@/assets/images/serivePgsImg/AnnualAccounts/cf.svg";
import py from "@/assets/images/serivePgsImg/AnnualAccounts/py.svg";
import vati from "@/assets/images/serivePgsImg/AnnualAccounts/vati.svg";
import waa from "@/assets/images/serivePgsImg/AnnualAccounts/waa.svg";
import wsa from "@/assets/images/serivePgsImg/AnnualAccounts/wsa.svg";
//mmmm
import statisticsone from "@/assets/images/serivePgsImg/statics/staticsone.svg";
import statisticstwo from "@/assets/images/serivePgsImg/statics/staticstwo.svg";
import statisticsthree from "@/assets/images/serivePgsImg/statics/staticsthree.svg";

export const heroData = {
  // ========== SERVICE 1: Annual Accounts ==========
  "Annual-Accounts": {
    title: "Preparing ",
    titleHighlight: "Annual Accounts",
    subtitle: "for Year-End",
    description: [
      "Preparing annual accounts can be time-consuming and complicated, but it doesn’t have to be. At online accountant, we provide annual accounts services designed to make the process seamless and stress-free. Our expert team ensures your year-end accounts are accurate, compliant, and submitted on time, helping you avoid penalties and maintain peace of mind.",
      "Whether you're a small business or a growing enterprise, we offer annual accounts service tailored to meet your unique needs. Let us handle your Ltd company accounts, so you can focus on running your business with confidence",
    ],
    buttonText: "Get Started Now",
    image: HONE.src,
    bgImage: HeroBackgroundImg.src,
    whatYouGet: {
      heading: "What You Will Get",
      whatYouGetcards: [
        {
          key: 1,
          img: parkoutline.src,
          title: "Comprehensive Online Accounting Services for UK Businesses",
          description:
            "Outsource Accounting offers a suite of core financial and accounting services designed to support businesses of all sizes. These include monthly bookkeeping, accounts preparation, corporation tax filing, self-assessment tax returns, payroll management, and VAT return submissions.",
        },
        {
          key: 2,
          img: affordableCleanEnergy.src,
          title: "Affordable Pricing with Transparent, Flat-Rate Fees",
          description:
            "One of the standout benefits highlighted on the website is clear and affordable pricing. Outsource Accounting emphasises flat-rate pricing with no hidden charges — meaning you know what you'll pay from the start, whether you're a small start-up or an established company.",
        },
        {
          key: 3,
          img: userexpert.src,
          title: "Expert Support from Professional UK Accountants",
          description:
            "Instead of automated bots or generic financial software, the services are delivered by a real team of expert accountants who provide personalised one-on-one support. This means you get direct access to professionals who can explain complex financial requirements.",
        },
        {
          key: 4,
          img: addbusiness.src,
          title: "Insightful Financial Reporting Tailored to Your Business",
          description:
            "Outsource Accounting doesn't just manage numbers — they provide insightful financial reporting that goes beyond basic bookkeeping. These reports are designed to give you a clearer picture of your company's financial health, helping you understand profitability, cash flow trends, and tax obligations.",
        },
        {
          key: 5,
          img: arttrack.src,
          title: "Supportive Mission and Proven Track Record",
          description:
            "According to the website's 'About Us' section, the company started with a simple mission: to make professional accounting affordable and accessible to all UK businesses. With over two decades of experience and more than 1,000 clients served, Outsource Accounting positions itself as a reliable and trustworthy partner committed to helping businesses.",
        },
        {
          key: 6,
          img: housecompliance.src,
          title: "Tax Filing, HMRC and Companies House Compliance",
          description:
            "A core part of the service is handling all regulatory filings required in the UK. This includes Corporation Tax returns, VAT return submissions, annual accounts filings with Companies House, and Self-Assessment tax returns for business owners. Outsource Accounting liaises with HM Revenue & Customs (HMRC) and Companies House on your behalf.",
        },
      ],
    },
    whyChooseUs: {
      heading: "Why Choose Us for Your UK Annual Accounts?",
      img: whyOutSourceUperImg.src,
      whyChooseUscards: [
        {
          key: 1,
          img: tickCircle.src,
          title: "Compliance You Can Trust",
          description:
            "We handle everything from preparing your financial statements to submitting them to HMRC and Companies House. Our experts stay up to date with the latest regulations, ensuring your statutory accounts and end of the year accounting are fully compliant and filed correctly.",
        },
        {
          key: 2,
          img: tickCircle.src,
          title: "No Hidden Costs:",
          description:
            "We believe in transparency. Our annual accounts services operate on a fixed pricing model—what we quote is what you pay, with no hidden fees.",
        },
        {
          key: 3,
          img: tickCircle.src,
          title: "Expert Advice, Anytime:",
          description:
            "Your company annual accounts service isn’t just a legal requirement—it’s an opportunity to gain insights into your financial health. We go beyond compliance, offering actionable advice to help you reduce tax liabilities and identify growth opportunities.",
        },
      ],
    },
    ServiceData: {
      heading: "Our Simple Process – ",
      highlightHeading: "How It Works",
      ServiceData: [
        {
          index: 1,
          imgSrc: gettingtoknow.src,
          title: "Getting to Know You",
          description:
            "Our first step is understanding your business and its specific needs. This ensures our annual accounts services are fully aligned with your objectives",
        },
        {
          index: 2,
          imgSrc: prepareaccount.src,
          title: "Preparing Your Accounts",
          description:
            "Using cloud-based accounting systems, we handle every detail, from your balance sheet to your Companies House annual report submission.",
        },
        {
          index: 3,
          imgSrc: reviewexplain.src,
          title: "Review & Explain",
          description:
            "We don’t just prepare your annual accounts return—we explain them. You’ll understand key figures, ensuring you have a clear picture of your financial health.",
        },
        {
          index: 4,
          imgSrc: everytime.src,
          title: "Submitting On Time, Every Time",
          description:
            "We guarantee timely submission of your Ltd company accounts to HMRC and Companies House, ensuring you never miss a deadline.",
        },
      ],
    },
    WhatData: {
      heading: "What Are Annual Accounts?",
      descriptionone:
        "Annual accounts are a formal set of financial statements prepared at the end of each financial year, giving a clear overview of a business’s financial performance and position. In the UK, they typically include a profit and loss account, balance sheet, and supporting notes, all prepared in line with HMRC and Companies House requirements. These accounts show how the business has performed over the year, helping owners understand profits, expenses, assets, and liabilities.",
      descriptiontwo:
        "Beyond compliance, annual accounts are an important tool for planning and growth. They help ensure accurate tax calculations, reduce the risk of penalties, and support transparent financial reporting. Well-prepared annual accounts also build confidence with lenders, investors, and stakeholders, while giving business owners the insights they need to make informed decisions and plan for the future.",
      img: acc.src,
    },
    WhoData: {
      heading: "Who Needs Annual Accounts?",
      descriptionone:
        " In the UK, limited companies are legally required to prepare annual accounts and submit them to both HMRC and Companies House. This applies to all limited companies, whether they are actively trading or dormant. Annual accounts ensure transparency, accurate tax reporting, and compliance with UK financial regulations",
      descriptiontwo:
        "Sole traders and partnerships may also need annual accounts, especially to calculate taxable profits and complete Self Assessment tax returns. While the legal requirements are different from limited companies, preparing annual accounts helps business owners understand their financial position, manage taxes effectively, and plan for future growth.",
      img: AnnualAccounts.src,
    },
    getstarted: {
      heading: "Let's Get Started",
      descriptionone:
        "Stop worrying about the complexity of preparing UK company accounts. Let us handle your annual accounts services while you focus on growing your business. Contact us today for a free consultation and experience the difference our expertise can make.",
      descriptiontwo:
        "Annual accounts are the key to setting and reaching both short- and long-term goals, and we’ll work with you every step of the way to help you stay on track.",
    },
    statics: {
      heading: "More Than Just Numbers",
      description:
        "Our company annual accounts service doesn’t end with filing. We provide insights that help you:",
      data: [
        {
          key: 1,
          img: tickCircle.src,
          heading: "",
          description: "Identify profitable areas.",
        },
        {
          key: 2,
          img: tickCircle.src,
          heading: "",
          description: "Minimise your tax liability.",
        },
        {
          key: 3,
          img: tickCircle.src,
          heading: "",
          description: "Make informed decisions to drive growth.",
        },
      ],
      imgone: statisticsone.src,
      imgtwo: statisticstwo.src,
      imgthree: statisticsthree.src,
    },
  },
  // ========== SERVICE 2: Self Assessment Tax ==========
  "Self-Assessment-Tax": {
    title: "File ",
    titleHighlight: "Self Assessment Tax",
    subtitle: "Return Online",
    description: [
      "Filing your Self Assessment tax return in the UK doesn’t have to be stressful. With us, we simplify the process of submitting your tax return online, ensuring accuracy, timeliness, and compliance with HMRC regulations. Whether you are self-employed, a freelancer, or an individual with additional income sources, our expert team is here to guide you through every step.",
      "The online submission of self-assessment tax returns offers a secure, efficient way to meet your tax obligations, saving you from the complexities of manual filing. filing a self-assessment tax return online minimises the chance of errors and allows quicker processing. With us, you can leverage our user-friendly online tools and expert guidance to ensure a seamless process from start to finish.",
    ],
    buttonText: "Get Started Now",
    image: HTOO.src,
    bgImage: HeroBackgroundImg.src,
    whatYouGet: {
      heading: "What You Will Get",
      whatYouGetcards: [
        {
          key: 1,
          img: parkoutline.src,
          title: "Comprehensive Online Accounting Services for UK Businesses",
          description:
            "Outsource Accounting offers a suite of core financial and accounting services designed to support businesses of all sizes. These include monthly bookkeeping, accounts preparation, corporation tax filing, self-assessment tax returns, payroll management, and VAT return submissions.",
        },
        {
          key: 2,
          img: affordableCleanEnergy.src,
          title: "Affordable Pricing with Transparent, Flat-Rate Fees",
          description:
            "One of the standout benefits highlighted on the website is clear and affordable pricing. Outsource Accounting emphasises flat-rate pricing with no hidden charges — meaning you know what you'll pay from the start, whether you're a small start-up or an established company.",
        },
        {
          key: 3,
          img: userexpert.src,
          title: "Expert Support from Professional UK Accountants",
          description:
            "Instead of automated bots or generic financial software, the services are delivered by a real team of expert accountants who provide personalised one-on-one support. This means you get direct access to professionals who can explain complex financial requirements.",
        },
        {
          key: 4,
          img: addbusiness.src,
          title: "Insightful Financial Reporting Tailored to Your Business",
          description:
            "Outsource Accounting doesn't just manage numbers — they provide insightful financial reporting that goes beyond basic bookkeeping. These reports are designed to give you a clearer picture of your company's financial health, helping you understand profitability, cash flow trends, and tax obligations.",
        },
        {
          key: 5,
          img: arttrack.src,
          title: "Supportive Mission and Proven Track Record",
          description:
            "According to the website's 'About Us' section, the company started with a simple mission: to make professional accounting affordable and accessible to all UK businesses. With over two decades of experience and more than 1,000 clients served, Outsource Accounting positions itself as a reliable and trustworthy partner committed to helping businesses.",
        },
        {
          key: 6,
          img: housecompliance.src,
          title: "Tax Filing, HMRC and Companies House Compliance",
          description:
            "A core part of the service is handling all regulatory filings required in the UK. This includes Corporation Tax returns, VAT return submissions, annual accounts filings with Companies House, and Self-Assessment tax returns for business owners. Outsource Accounting liaises with HM Revenue & Customs (HMRC) and Companies House on your behalf.",
        },
      ],
    },
    whyChooseUs: {
      heading: "Why Choose Affotax for Your Self Assessment?",
      img: selfassesment.src,
      whyChooseUscards: [
        {
          key: 1,
          img: tickCircle.src,
          title: "Ease of Use",
          description:
            "Our simple, user-friendly online service guides you through every step of the Self Assessment process. No confusing jargon—just clear, concise instructions",
        },
        {
          key: 2,
          img: tickCircle.src,
          title: "Expert Support",
          description:
            "If you have any questions, our team of tax professionals is available to provide guidance and help resolve any issues.",
        },
        {
          key: 3,
          img: tickCircle.src,
          title: "Accuracy and Compliance",
          description:
            "We meticulously check your return to ensure it meets all of HMRC's requirements. With us, you can be confident that your tax return is error-free and fully compliant.",
        },
        {
          key: 4,
          img: tickCircle.src,
          title: "Timeliness",
          description:
            "We ensure your Self Assessment is filed on time, every time—avoiding penalties and ensuring you meet your tax obligations with ease.",
        },
      ],
    },
    ServiceData: {
      heading: "Step-by-Step Submission",
      highlightHeading: "",
      ServiceData: [
        {
          index: 1,
          imgSrc: gettingtoknow.src,
          title: "Gather Relevant Documents :",
          description:
            " To ensure accurate data entry, compile essential documents like income statements, expense records, bank interest details, and pension contributions. Our checklist can help you organise these documents efficiently..",
        },
        {
          index: 2,
          imgSrc: prepareaccount.src,
          title: "Complete Your SA100 Tax Form",
          description:
            "Our team assists you in filling out the SA100 tax form accurately, capturing all relevant income and allowable expenses. We’ll advise on any tax reliefs and deductions applicable to maximise your returns and minimise liabilities.",
        },
        {
          index: 3,
          imgSrc: reviewexplain.src,
          title: "Online Submission of Self Assessment Tax",
          description:
            "Once your form is complete and reviewed, we file your self-assessment tax return online directly to HMRC. By handling this process online, we ensure quicker processing and verification.",
        },
        {
          index: 4,
          imgSrc: everytime.src,
          title: "Receive Confirmation and Stay Updated",
          description:
            " After submission, we provide you with confirmation and follow up with any updates from HMRC. Should there be any further actions required, Online Accountant remains by your side to address them promptly.",
        },
      ],
    },
    WhatData: {
      heading: "What Is Self Assessment?",
      descriptionone:
        "Self Assessment is the UK tax system used by HMRC to ensure individuals and businesses pay the correct amount of Income Tax. Unlike employees whose taxes are deducted automatically through PAYE (Pay As You Earn), Self Assessment requires taxpayers to report their own income, expenses, and any other financial activities over the tax year.",
      descriptiontwo:
        "By completing a Self Assessment tax return, HMRC can calculate exactly how much tax is owed based on all sources of income, including employment, self-employment, rental income, dividends, and savings. This system allows taxpayers to take responsibility for accurately declaring their income and claiming eligible deductions, ensuring compliance with UK tax laws. Deadlines are strict: paper returns must be submitted by 31st October and online returns by 31st January following the end of the tax year, with penalties for late submissions or underpayment.",
      img: wsa.src,
    },
    WhoData: {
      heading: "Who Needs to Complete a Self Assessment?",
      descriptionone:
        "In the UK, several groups of people are required to file a Self Assessment tax return. Sole traders and self-employed individuals must report all business income and expenses to calculate taxable profits. Business partners in partnerships also need to declare their share of profits",
      descriptiontwo:
        "Additionally, individuals with extra sources of income, such as rental income, freelance work, dividends from investments, foreign income, or capital gains, are also obligated to complete a Self Assessment. Certain company directors, high earners, and those claiming specific tax reliefs may also be required to file. Even if tax has been paid through PAYE, individuals may need to submit a return to claim allowances, report additional income, or correct any discrepancies. Filing Self Assessment on time ensures compliance, avoids penalties, and provides an accurate record of income for future financial planning.",
      img: selfassessment.src,
    },
    getstarted: {
      heading: "Let's Get Started",
      descriptionone:
        "Let Us take the stress out of filing your self-assessment tax return. Our team will ensure that your SA100 form is completed and submitted accurately, on time, and with all the necessary information. We make sure everything is in order, so you don’t have to worry about anything.",
      descriptiontwo:
        "Contact Us  now for expert support with your self-assessment tax return. We’ll help you navigate the process efficiently, so you can focus on your personal and professional goals without the worry of tax compliance.",
    },
    statics: {
      heading: "Who Needs to File an SA100 Tax Return?",
      description:
        "Certain individuals are legally required to submit a self-assessment tax return. You’ll need to complete and submit this form if you:",
      imgone: statisticsone.src,
      imgtwo: statisticstwo.src,
      imgthree: statisticsthree.src,
      data: [
        {
          key: 1,
          img: tickCircle.src,
          heading: "",
          description: "Are self-employed or a freelancer",
        },
        {
          key: 2,
          img: tickCircle.src,
          heading: "",
          description: "Are a company director",
        },
        {
          key: 3,
          img: tickCircle.src,
          heading: "",
          description: "Earn over £100,000 annually",
        },
        {
          key: 4,
          img: tickCircle.src,
          heading: "",
          description: "Have received dividends or capital gains",
        },
        {
          key: 5,
          img: tickCircle.src,
          description:
            "Have paid capital gains tax or the High-Income Child Benefit Charge",
        },
        {
          key: 6,
          img: tickCircle.src,
          heading: "",
          description:
            "Have untaxed income, such as rental income, interest, or savings",
        },
      ],
    },
  },
  // ========== SERVICE 3: Bookkeeping-Services ==========
  "Bookkeeping-Services": {
    title: "Reliable ",
    titleHighlight: "Bookkeeping",
    subtitle: "Services",
    description: [
      "Managing your finances shouldn’t be stressful. At outsource, we specialize in offering bookkeeping UK services that simplify financial management for businesses of all sizes. Our expert team ensures your records are organized, compliant, and always up-to-date, allowing you to focus on the growth of your business while we handle the numbers.",
      "Whether you need expert help with online bookkeeping, are searching for bookkeepers near me, or need assistance with double entry bookkeeping, Outsource delivers customized solutions that fit your unique needs. Our aim is to provide businesses across the UK with accessible, efficient, and error-free financial management services.",
    ],
    buttonText: "Get Started Now",
    image: HTHREE.src,
    bgImage: HeroBackgroundImg.src,
    whatYouGet: {
      heading: "What You Will Get",
      whatYouGetcards: [
        {
          key: 1,
          img: parkoutline.src,
          title: "Comprehensive Online Accounting Services for UK Businesses",
          description:
            "Outsource Accounting offers a suite of core financial and accounting services designed to support businesses of all sizes. These include monthly bookkeeping, accounts preparation, corporation tax filing, self-assessment tax returns, payroll management, and VAT return submissions.",
        },
        {
          key: 2,
          img: affordableCleanEnergy.src,
          title: "Affordable Pricing with Transparent, Flat-Rate Fees",
          description:
            "One of the standout benefits highlighted on the website is clear and affordable pricing. Outsource Accounting emphasises flat-rate pricing with no hidden charges — meaning you know what you'll pay from the start, whether you're a small start-up or an established company.",
        },
        {
          key: 3,
          img: userexpert.src,
          title: "Expert Support from Professional UK Accountants",
          description:
            "Instead of automated bots or generic financial software, the services are delivered by a real team of expert accountants who provide personalised one-on-one support. This means you get direct access to professionals who can explain complex financial requirements.",
        },
        {
          key: 4,
          img: addbusiness.src,
          title: "Insightful Financial Reporting Tailored to Your Business",
          description:
            "Outsource Accounting doesn't just manage numbers — they provide insightful financial reporting that goes beyond basic bookkeeping. These reports are designed to give you a clearer picture of your company's financial health, helping you understand profitability, cash flow trends, and tax obligations.",
        },
        {
          key: 5,
          img: arttrack.src,
          title: "Supportive Mission and Proven Track Record",
          description:
            "According to the website's 'About Us' section, the company started with a simple mission: to make professional accounting affordable and accessible to all UK businesses. With over two decades of experience and more than 1,000 clients served, Outsource Accounting positions itself as a reliable and trustworthy partner committed to helping businesses.",
        },
        {
          key: 6,
          img: housecompliance.src,
          title: "Tax Filing, HMRC and Companies House Compliance",
          description:
            "A core part of the service is handling all regulatory filings required in the UK. This includes Corporation Tax returns, VAT return submissions, annual accounts filings with Companies House, and Self-Assessment tax returns for business owners. Outsource Accounting liaises with HM Revenue & Customs (HMRC) and Companies House on your behalf.",
        },
      ],
    },
    whyChooseUs: {
      heading: "Why Digital Bookkeeping?",
      img: bookkeeping.src,
      whyChooseUscards: [
        {
          key: 1,
          img: tickCircle.src,
          title: "Access Your Financial Data Anywhere, Anytime:",
          description:
            "Our cloud-based solutions give you real-time access to your financial data, no matter where you are or what device you use.",
        },
        {
          key: 2,
          img: tickCircle.src,
          title: "Go Paperless:",
          description:
            "Say goodbye to manual paperwork. All of your financial records are stored digitally in secure, user-friendly apps, making it easier to organize and access them when needed",
        },
        {
          key: 3,
          img: tickCircle.src,
          title: "Automate Routine Tasks:",
          description:
            "With automated processes like invoicing, bank reconciliation, and financial reporting, our system reduces the potential for human error, saving you time and increasing efficiency.",
        },
      ],
    },
    ServiceData: {
      heading: "Why Choose Us for –",
      highlightHeading: " Your UK Bookkeeping Needs?",
      ServiceData: [
        {
          index: 1,
          imgSrc: gettingtoknow.src,
          title: "Experienced Team",
          description:
            "Our experts are highly skilled in double entry bookkeeping and proficient in all major accounting software.",
        },
        {
          index: 2,
          imgSrc: prepareaccount.src,
          title: "Complete Your SA100 Tax Form",
          description:
            "We ensure your financial records are fully compliant with UK regulations, reducing the risk of penalties.",
        },
        {
          index: 3,
          imgSrc: reviewexplain.src,
          title: "Online Submission of Self Assessment Tax",
          description:
            "Our services are customized to meet the specific needs of your business.",
        },
        {
          index: 4,
          imgSrc: everytime.src,
          title: "Receive Confirmation and Stay Updated",
          description:
            "Enjoy clear, upfront pricing for all services—no hidden fees, just reliable, straightforward bookkeeping.",
        },
      ],
    },
    WhatData: {
      heading: "What Is Bookkeeping?",
      descriptionone:
        "Bookkeeping is the process of recording, organizing, and maintaining all financial transactions of a business. It involves tracking income, expenses, sales, purchases, and other financial activities to provide a clear and accurate picture of a company’s financial health. In the UK, bookkeeping is essential not only for day-to-day business management but also for tax compliance, including filing annual accounts and completing Self Assessment tax returns. Proper bookkeeping ensures that all financial records are up to date, helps businesses monitor cash flow, manage budgets effectively, and make informed decisions. Accurate bookkeeping also makes it easier to prepare financial statements, identify business trends, and avoid errors or discrepancies that could lead to penalties from HMRC.",
      descriptiontwo:
        "Bookkeeping can be done manually or using accounting software, depending on the size and complexity of the business. It typically includes recording receipts, invoices, bank statements, payroll transactions, and any other financial activity.",
      img: bk.src,
    },
    WhoData: {
      heading: "Who Needs Bookkeeping in the UK?",
      descriptionone:
        "In the UK, anyone running a business—from sole traders and freelancers to partnerships and limited companies—needs bookkeeping. For self-employed individuals and sole traders, bookkeeping is essential to calculate profits and losses accurately, which are then used to complete a Self Assessment tax return. Partnerships must also maintain records of each partner’s share of income and expenses.",
      descriptiontwo:
        "Limited companies are legally required to maintain bookkeeping records to prepare annual accounts, submit Corporation Tax returns, and comply with Companies House regulations. Even individuals with additional sources of income, such as rental properties, investments, or freelance work, benefit from bookkeeping to track income and expenses efficiently. Without proper bookkeeping, it becomes difficult to calculate tax obligations accurately, claim allowable expenses, or make informed business decisions, which can lead to fines, penalties, or missed growth opportunities.",
      img: bookeeping.src,
    },
    getstarted: {
      heading: "Let's Get Started",
      descriptionone:
        "Let Us take the stress out of managing your finances. Whether you need help with annual bookkeeping or require online bookkeeping solutions, we’re here to simplify your financial processes and keep your business on track.",
      descriptiontwo:
        "Contact us now to book your free consultation or get an instant quote. Let us help you focus on what matters most—growing your business.",
    },
    statics: {
      heading: "How Our Bookkeeping Service Works",
      description: "Getting started with Outsource is simple and hassle-free:",
      imgone: statisticsone.src,
      imgtwo: statisticstwo.src,
      imgthree: statisticsthree.src,
      data: [
        {
          key: 1,
          img: tickCircle.src,
          heading: "Free Consultation:",
          description:
            "We’ll discuss your bookkeeping UK needs and tailor our services to your requirements.",
        },
        {
          key: 2,
          img: tickCircle.src,
          heading: "Data Collection:",
          description:
            "We securely gather your receipts, invoices, and bank statements to ensure accurate record-keeping.",
        },
        {
          key: 3,
          img: tickCircle.src,
          heading: "Bookkeeping & Reporting:",
          description:
            "Our team handles the daily bookkeeping and provides you with regular updates to keep you informed of your financial standing.",
        },
        {
          key: 4,
          img: tickCircle.src,
          heading: "Ongoing Support:",
          description:
            "Our team is always available for any questions, ensuring you receive continuous support and that your finances remain in excellent shape.",
        },
      ],
    },
  },
  // ========== SERVICE 4: Payroll Services ==========
  "Payroll-services": {
    title: "Efficient",
    titleHighlight: "Payroll Services",
    subtitle: "Tax Return Services",
    description: [
      "Small businesses spend roughly six hours per month sorting their own payroll, this is time you can’t afford to waste.",
      "Payroll outsourcing is the way forward.  Let the team of expert accountants at Online Accountant manage the process for you with our online payroll services. Payroll outsourcing eliminates costly mistakes as the whole process will be managed by a qualified accountant.",
    ],
    buttonText: "Get Started Now",
    image: HFOOR.src,
    bgImage: HeroBackgroundImg.src,
    whatYouGet: {
      heading: "What You Will Get",
      whatYouGetcards: [
        {
          key: 1,
          img: parkoutline.src,
          title: "Comprehensive Online Accounting Services for UK Businesses",
          description:
            "Outsource Accounting offers a suite of core financial and accounting services designed to support businesses of all sizes. These include monthly bookkeeping, accounts preparation, corporation tax filing, self-assessment tax returns, payroll management, and VAT return submissions.",
        },
        {
          key: 2,
          img: affordableCleanEnergy.src,
          title: "Affordable Pricing with Transparent, Flat-Rate Fees",
          description:
            "One of the standout benefits highlighted on the website is clear and affordable pricing. Outsource Accounting emphasises flat-rate pricing with no hidden charges — meaning you know what you'll pay from the start, whether you're a small start-up or an established company.",
        },
        {
          key: 3,
          img: userexpert.src,
          title: "Expert Support from Professional UK Accountants",
          description:
            "Instead of automated bots or generic financial software, the services are delivered by a real team of expert accountants who provide personalised one-on-one support. This means you get direct access to professionals who can explain complex financial requirements.",
        },
        {
          key: 4,
          img: addbusiness.src,
          title: "Insightful Financial Reporting Tailored to Your Business",
          description:
            "Outsource Accounting doesn't just manage numbers — they provide insightful financial reporting that goes beyond basic bookkeeping. These reports are designed to give you a clearer picture of your company's financial health, helping you understand profitability, cash flow trends, and tax obligations.",
        },
        {
          key: 5,
          img: arttrack.src,
          title: "Supportive Mission and Proven Track Record",
          description:
            "According to the website's 'About Us' section, the company started with a simple mission: to make professional accounting affordable and accessible to all UK businesses. With over two decades of experience and more than 1,000 clients served, Outsource Accounting positions itself as a reliable and trustworthy partner committed to helping businesses.",
        },
        {
          key: 6,
          img: housecompliance.src,
          title: "Tax Filing, HMRC and Companies House Compliance",
          description:
            "A core part of the service is handling all regulatory filings required in the UK. This includes Corporation Tax returns, VAT return submissions, annual accounts filings with Companies House, and Self-Assessment tax returns for business owners. Outsource Accounting liaises with HM Revenue & Customs (HMRC) and Companies House on your behalf.",
        },
      ],
    },
    whyChooseUs: {
      heading: "Comprehensive Payroll",
      img: payroll.src,
      whyChooseUscards: [
        {
          key: 1,
          img: tickCircle.src,
          title: "Accurate Payroll Calculations:",
          description:
            "We ensure accurate calculations for income tax and National Insurance deductions.",
        },
        {
          key: 2,
          img: tickCircle.src,
          title: "Monthly HMRC Returns:",
          description:
            "We take care of all HMRC submissions, ensuring you stay compliant throughout the year.",
        },
        {
          key: 3,
          img: tickCircle.src,
          title: "Electronic Pay slips:",
          description:
            "Your pay slips are emailed directly to you, making it easy to access your financial records.",
        },
        {
          key: 4,
          img: tickCircle.src,
          title: "Year-End Processing:",
          description:
            "At the end of the year, we prepare and submit P60s for each director, ensuring all year-end obligations are met.",
        },
      ],
    },
    ServiceData: {
      heading: "Benefits of Choosing Us for",
      highlightHeading: "Payroll Service",
      ServiceData: [
        {
          index: 1,
          imgSrc: gettingtoknow.src,
          title: "Expertise in Payroll Solutions",
          description:
            "Our team specialises in providing payroll for directors, ensuring you get the best advice and support.",
        },
        {
          index: 2,
          imgSrc: prepareaccount.src,
          title: "Cost-Effective Pricing",
          description:
            "Enjoy our services starting at an affordable price, which includes all necessary payroll process.",
        },
        {
          index: 3,
          imgSrc: reviewexplain.src,
          title: "Tax Efficiency",
          description:
            "Setting up a payroll can be more tax-efficient than taking dividends alone.",
        },
        {
          index: 4,
          imgSrc: everytime.src,
          title: "Hassle-Free Setup",
          description:
            "Our straightforward setup process means you won’t encounter hidden charges.",
        },
      ],
    },
    WhatData: {
      heading: "What Is Payroll?",
      descriptionone:
        "Payroll is the process of managing and processing employee payments within a business. It involves calculating salaries or wages, deducting income tax (PAYE), National Insurance contributions, pension contributions, and any other statutory deductions, and ensuring employees are paid accurately and on time. In the UK, payroll also includes keeping detailed records of payments, benefits, and tax deductions, submitting reports to HMRC, and ensuring compliance with employment and tax laws.",
      descriptiontwo:
        "Efficient payroll management is critical for maintaining employee trust, avoiding legal issues, and ensuring the business meets its tax obligations. Payroll systems often integrate with bookkeeping and accounting software to track costs, calculate deductions automatically, and generate payslips and reports. Proper payroll management also supports statutory requirements, such as Real Time Information (RTI) submissions to HMRC, ensuring that taxes and National Insurance contributions are reported correctly each time employees are paid.",
      img: py.src,
    },
    WhoData: {
      heading: "Who Needs Payroll in the UK?",
      descriptionone:
        "In the UK, any business that employs staff must manage payroll. This includes limited companies, partnerships, and sole traders who hire employees. Businesses are legally required to operate PAYE (Pay As You Earn) for all employees, which ensures the correct income tax and National Insurance contributions are deducted and paid to HMRC. Payroll is essential not only for compliance but also for record-keeping, preparing financial statements, and calculating accurate figures for annual accounts and Self Assessment tax returns where applicable.",
      descriptiontwo:
        "Even small businesses with a single employee must maintain payroll records, provide payslips, and submit RTI reports to HMRC. Freelancers or self-employed individuals without employees generally do not need payroll, but anyone with staff—full-time, part-time, or temporary—must manage payroll carefully. Proper payroll ensures accurate tax reporting, avoids fines or penalties from HMRC, and helps business owners maintain transparency and trust with their employees.",
      img: payrol.src,
    },
    getstarted: {
      heading: "Let's Get Started",
      descriptionone:
        "If your business changes, such as stopping employment or if your employees' earnings fall below the necessary threshold, we can assist you with closing or adjusting your employer's PAYE registration.",
      descriptiontwo:
        "Register for PAYE online with Us today to ensure your business is compliant with UK payroll and tax regulations. We offer reliable support to get you registered quickly and smoothly.",
    },
    statics: {
      heading: "How to Register for PAYE with Us",
      description: "Getting started with Outsource is simple and hassle-free:",
      imgone: statisticsone.src,
      imgtwo: statisticstwo.src,
      imgthree: statisticsthree.src,
      data: [
        {
          key: 1,
          img: tickCircle.src,
          heading: "",
          description:
            "If you're forming a new company, you can order our PAYE registration service as part of your company formation package.",
        },
        {
          key: 2,
          img: tickCircle.src,
          heading: "",
          description:
            "Already have a company? Simply log in to your Affotax account and select register for payroll from our services, then complete your purchase.",
        },
      ],
    },
  },
  // ========== SERVICE 5: vat-returns ==========
  "vat-returns": {
    title: "Expert ",
    titleHighlight: "VAT",
    subtitle: "Services",
    description: [
      "Simplify your VAT registration process with Online accountant expert-driven VAT registration service. Whether you’re registering voluntarily or out of necessity, our streamlined process ensures compliance with HMRC VAT online registration requirements, saving you time and hassle.",
      "VAT (Value Added Tax) is a tax on goods and services in the UK. Businesses must register for VAT if their annual turnover exceeds £85,000. Registering allows you to charge VAT on your sales and reclaim VAT on business purchases.",
    ],
    buttonText: "Get Started Now",
    image: HFIVE.src,
    bgImage: HeroBackgroundImg.src,
    whatYouGet: {
      heading: "What You Will Get",
      whatYouGetcards: [
        {
          key: 1,
          img: parkoutline.src,
          title: "Comprehensive Online Accounting Services for UK Businesses",
          description:
            "Outsource Accounting offers a suite of core financial and accounting services designed to support businesses of all sizes. These include monthly bookkeeping, accounts preparation, corporation tax filing, self-assessment tax returns, payroll management, and VAT return submissions.",
        },
        {
          key: 2,
          img: affordableCleanEnergy.src,
          title: "Affordable Pricing with Transparent, Flat-Rate Fees",
          description:
            "One of the standout benefits highlighted on the website is clear and affordable pricing. Outsource Accounting emphasises flat-rate pricing with no hidden charges — meaning you know what you'll pay from the start, whether you're a small start-up or an established company.",
        },
        {
          key: 3,
          img: userexpert.src,
          title: "Expert Support from Professional UK Accountants",
          description:
            "Instead of automated bots or generic financial software, the services are delivered by a real team of expert accountants who provide personalised one-on-one support. This means you get direct access to professionals who can explain complex financial requirements.",
        },
        {
          key: 4,
          img: addbusiness.src,
          title: "Insightful Financial Reporting Tailored to Your Business",
          description:
            "Outsource Accounting doesn't just manage numbers — they provide insightful financial reporting that goes beyond basic bookkeeping. These reports are designed to give you a clearer picture of your company's financial health, helping you understand profitability, cash flow trends, and tax obligations.",
        },
        {
          key: 5,
          img: arttrack.src,
          title: "Supportive Mission and Proven Track Record",
          description:
            "According to the website's 'About Us' section, the company started with a simple mission: to make professional accounting affordable and accessible to all UK businesses. With over two decades of experience and more than 1,000 clients served, Outsource Accounting positions itself as a reliable and trustworthy partner committed to helping businesses.",
        },
        {
          key: 6,
          img: housecompliance.src,
          title: "Tax Filing, HMRC and Companies House Compliance",
          description:
            "A core part of the service is handling all regulatory filings required in the UK. This includes Corporation Tax returns, VAT return submissions, annual accounts filings with Companies House, and Self-Assessment tax returns for business owners. Outsource Accounting liaises with HM Revenue & Customs (HMRC) and Companies House on your behalf.",
        },
      ],
    },
    whatYouGet: {
      heading: "What You Will Get",
      whatYouGetcards: [
        {
          key: 1,
          img: parkoutline.src,
          title: "Comprehensive Online Accounting Services for UK Businesses",
          description:
            "Outsource Accounting offers a suite of core financial and accounting services designed to support businesses of all sizes. These include monthly bookkeeping, accounts preparation, corporation tax filing, self-assessment tax returns, payroll management, and VAT return submissions.",
        },
        {
          key: 2,
          img: affordableCleanEnergy.src,
          title: "Affordable Pricing with Transparent, Flat-Rate Fees",
          description:
            "One of the standout benefits highlighted on the website is clear and affordable pricing. Outsource Accounting emphasises flat-rate pricing with no hidden charges — meaning you know what you'll pay from the start, whether you're a small start-up or an established company.",
        },
        {
          key: 3,
          img: userexpert.src,
          title: "Expert Support from Professional UK Accountants",
          description:
            "Instead of automated bots or generic financial software, the services are delivered by a real team of expert accountants who provide personalised one-on-one support. This means you get direct access to professionals who can explain complex financial requirements.",
        },
        {
          key: 4,
          img: addbusiness.src,
          title: "Insightful Financial Reporting Tailored to Your Business",
          description:
            "Outsource Accounting doesn't just manage numbers — they provide insightful financial reporting that goes beyond basic bookkeeping. These reports are designed to give you a clearer picture of your company's financial health, helping you understand profitability, cash flow trends, and tax obligations.",
        },
        {
          key: 5,
          img: arttrack.src,
          title: "Supportive Mission and Proven Track Record",
          description:
            "According to the website's 'About Us' section, the company started with a simple mission: to make professional accounting affordable and accessible to all UK businesses. With over two decades of experience and more than 1,000 clients served, Outsource Accounting positions itself as a reliable and trustworthy partner committed to helping businesses.",
        },
        {
          key: 6,
          img: housecompliance.src,
          title: "Tax Filing, HMRC and Companies House Compliance",
          description:
            "A core part of the service is handling all regulatory filings required in the UK. This includes Corporation Tax returns, VAT return submissions, annual accounts filings with Companies House, and Self-Assessment tax returns for business owners. Outsource Accounting liaises with HM Revenue & Customs (HMRC) and Companies House on your behalf.",
        },
      ],
    },
    whyChooseUs: {
      heading: "Why Choose Us for VAT Registration Online?",
      img: vatreturns.src,
      whyChooseUscards: [
        {
          key: 1,
          img: tickCircle.src,
          title: "Expert Guidance:",
          description:
            "Our VAT specialists are available to assist with queries and ensure accurate registration.",
        },
        {
          key: 2,
          img: tickCircle.src,
          title: "HMRC Compliance:",
          description:
            "All documentation and filings meet HMRC VAT online regulations, avoiding delays or penalties.",
        },
        {
          key: 3,
          img: tickCircle.src,
          title: "Fast Processing:",
          description:
            "Get your VAT registration number within 7-10 working days.",
        },
      ],
    },
    ServiceData: {
      heading: "How Our  VAT Return Service Works",
      highlightHeading: "",
      ServiceData: [
        {
          index: 1,
          imgSrc: gettingtoknow.src,
          title: "Data Collection:",
          description:
            "We gather all the necessary information, including sales invoices, purchase receipts, and other relevant financial data.",
        },
        {
          index: 2,
          imgSrc: prepareaccount.src,
          title: "Cost-Effective Pricing",
          description:
            "Our experts prepare your VAT return, ensuring all figures are accurately calculated based on your business activities over the past quarter.",
        },
        {
          index: 3,
          imgSrc: reviewexplain.src,
          title: "Tax Efficiency",
          description:
            "Once your VAT return is prepared, we will submit it to HMRC on your behalf, ensuring compliance and timely filing.",
        },
        {
          index: 4,
          imgSrc: everytime.src,
          title: "Hassle-Free Setup",
          description:
            "After submission, we provide you with confirmation and any necessary documentation for your records.",
        },
      ],
    },
    WhatData: {
      heading: "What Is VAT?",
      descriptionone:
        "VAT (Value Added Tax) is a consumption tax applied to most goods and services sold in the UK. It is collected by businesses on behalf of HMRC and added to the sale price of products or services. Businesses that are VAT-registered must charge VAT to their customers, record it in their accounts, and submit regular VAT returns to HMRC, showing the VAT collected and paid on purchases.",
      descriptiontwo:
        "VAT plays a critical role in maintaining business compliance and financial transparency. Keeping accurate VAT records ensures businesses can reclaim VAT on allowable purchases and avoid fines or penalties for incorrect reporting. It also helps with cash flow management and financial planning. VAT is applicable at different rates depending on the product or service—standard, reduced, or zero rate—and businesses must ensure they apply the correct rate and maintain supporting documentation for each transaction.",
      img: vati.src,
    },
    WhoData: {
      heading: "Who Needs VAT in the UK?",
      descriptionone:
        "In the UK, businesses must register for VAT if their taxable turnover exceeds the current threshold set by HMRC (currently £85,000 per year). This applies to limited companies, sole traders, partnerships, and other entities providing taxable goods or services. Businesses with turnover below the threshold can also choose to register voluntarily, which can provide advantages such as reclaiming VAT on business expenses.",
      descriptiontwo:
        "VAT-registered businesses are responsible for charging VAT correctly, maintaining detailed VAT records, and submitting accurate VAT returns. This process ties into Self Assessment for business owners, particularly for sole traders and partnerships, as VAT information impacts overall taxable profits. Non-compliance or incorrect VAT filings can result in penalties, interest charges, or HMRC investigations, making accurate VAT management essential for any UK business dealing with taxable goods or services.",
      img: AnnualAccounts.src,
    },
    getstarted: {
      heading: "Let's Get Started",
      descriptionone:
        "Let Us handle your VAT online registration efficiently and accurately. With our expertise, you can focus on growing your business while we ensure compliance with HMRC requirements.",
      descriptiontwo:
        "Register for VAT online now to simplify the process and unlock the benefits of being VAT-registered.",
    },
    statics: {
      heading: "How to Register for VAT Online with Us",
      description:
        "Our online VAT registration UK service is simple and efficient. Follow these steps to get started:",
      imgone: statisticsone.src,
      imgtwo: statisticstwo.src,
      imgthree: statisticsthree.src,
      data: [
        {
          key: 1,
          img: tickCircle.src,
          heading: "Order Online: ",
          description: "Click the 'Register Now' button to begin.",
        },
        {
          key: 2,
          img: tickCircle.src,
          heading: "Submit Business Details:",
          description:
            "Complete our secure VAT questionnaire to provide the necessary information",
        },
        {
          key: 3,
          img: tickCircle.src,
          heading: "Application Submission",
          description:
            "We handle your VAT application online, submitting it directly to HMRC.",
        },
        {
          key: 4,
          img: tickCircle.src,
          heading: "Receive Confirmation:",
          description:
            "Within 7–10 working days, receive your VAT registration number and certificate by post.",
        },
      ],
    },
  },
  // ========== SERVICE 6: Company Formation ==========
  "company-formation": {
    title: "Seamless",
    titleHighlight: "Company Formation",
    subtitle: "",
    description: [
      "Setting up a UK company as a non-resident is simple and straightforward with outsource accounting. Whether you’re an entrepreneur expanding globally or seeking access to the thriving UK market, our hassle-free process ensures you can establish your presence with ease. Our service is designed to cater specifically to non-UK residents, offering you everything you need to start your business smoothly, including fast registration and a prestigious London office address.",
      "",
    ],
    buttonText: "Get Started Now",
    image: HSIX.src,
    bgImage: HeroBackgroundImg.src,
    whatYouGet: {
      heading: "What You Will Get",
      whatYouGetcards: [
        {
          key: 1,
          img: parkoutline.src,
          title: "Comprehensive Online Accounting Services for UK Businesses",
          description:
            "Outsource Accounting offers a suite of core financial and accounting services designed to support businesses of all sizes. These include monthly bookkeeping, accounts preparation, corporation tax filing, self-assessment tax returns, payroll management, and VAT return submissions.",
        },
        {
          key: 2,
          img: affordableCleanEnergy.src,
          title: "Affordable Pricing with Transparent, Flat-Rate Fees",
          description:
            "One of the standout benefits highlighted on the website is clear and affordable pricing. Outsource Accounting emphasises flat-rate pricing with no hidden charges — meaning you know what you'll pay from the start, whether you're a small start-up or an established company.",
        },
        {
          key: 3,
          img: userexpert.src,
          title: "Expert Support from Professional UK Accountants",
          description:
            "Instead of automated bots or generic financial software, the services are delivered by a real team of expert accountants who provide personalised one-on-one support. This means you get direct access to professionals who can explain complex financial requirements.",
        },
        {
          key: 4,
          img: addbusiness.src,
          title: "Insightful Financial Reporting Tailored to Your Business",
          description:
            "Outsource Accounting doesn't just manage numbers — they provide insightful financial reporting that goes beyond basic bookkeeping. These reports are designed to give you a clearer picture of your company's financial health, helping you understand profitability, cash flow trends, and tax obligations.",
        },
        {
          key: 5,
          img: arttrack.src,
          title: "Supportive Mission and Proven Track Record",
          description:
            "According to the website's 'About Us' section, the company started with a simple mission: to make professional accounting affordable and accessible to all UK businesses. With over two decades of experience and more than 1,000 clients served, Outsource Accounting positions itself as a reliable and trustworthy partner committed to helping businesses.",
        },
        {
          key: 6,
          img: housecompliance.src,
          title: "Tax Filing, HMRC and Companies House Compliance",
          description:
            "A core part of the service is handling all regulatory filings required in the UK. This includes Corporation Tax returns, VAT return submissions, annual accounts filings with Companies House, and Self-Assessment tax returns for business owners. Outsource Accounting liaises with HM Revenue & Customs (HMRC) and Companies House on your behalf.",
        },
      ],
    },
    whyChooseUs: {
      heading: "Key Features of Our UK Company Registration Service",
      img: companyformation.src,
      whyChooseUscards: [
        {
          key: 1,
          img: tickCircle.src,
          title: "Efficient Registration Process:",
          description:
            "Hassle-Free Registration Secure Documentation Name Check Assistance",
        },
        {
          key: 2,
          img: tickCircle.src,
          title: "Business Essentials for Startups:",
          description: "Free Domain Names Marketing Credits Business Listings",
        },
        {
          key: 3,
          img: tickCircle.src,
          title: "Banking Solutions:",
          description: "Business Account Assistance Prepaid Credit Card",
        },
      ],
    },
    ServiceData: {
      heading: "Why Choose Us for Limited Company Formation?",
      highlightHeading: "",
      ServiceData: [
        {
          index: 1,
          imgSrc: gettingtoknow.src,
          title: "Simplified Process",
          description:
            "Setting up a limited company in the UK has never been easier. Complete your UK company registration in just minutes, with most applications approved within 1-3 business days.",
        },
        {
          index: 2,
          imgSrc: prepareaccount.src,
          title: "Exclusive Benefits:",
          description:
            "Access over £150 worth of free tools and rewards to help kickstart your business journey.",
        },
        {
          index: 3,
          imgSrc: reviewexplain.src,
          title: "Legal Compliance:",
          description:
            " Our expert team ensures that every step of your limited company formation complies with Companies House regulations",
        },
        {
          index: 4,
          imgSrc: everytime.src,
          title: "Trusted Service",
          description:
            "Join thousands of satisfied entrepreneurs who trust outsource accounting for their business needs.",
        },
      ],
    },
    WhatData: {
      heading: "What Is Company Formation?",
      descriptionone:
        "Company formation is the legal process of creating a new company as a separate entity under UK law. It involves registering the company with Companies House, choosing a company name, defining its structure (such as private limited company – Ltd), and preparing essential documents like the Memorandum and Articles of Association. Once a company is formed, it becomes a legal entity distinct from its owners, meaning it can own assets, enter contracts, incur liabilities, and pay taxes in its own name.",
      descriptiontwo:
        "Company formation is a crucial step for anyone looking to establish a professional business presence, protect personal assets, and benefit from limited liability. Proper formation ensures the company complies with UK laws, meets statutory reporting requirements, and is recognised by banks, investors, and HMRC. It also provides credibility with clients and suppliers and forms the foundation for business growth, financial planning, and tax management.",
      img: cf.src,
    },
    WhoData: {
      heading: "Who Needs Company Formation in the UK?",
      descriptionone:
        "In the UK, anyone planning to start a business as a limited company must go through the company formation process. This includes entrepreneurs, small business owners, and start-ups who want to separate their personal finances from business liabilities. Unlike sole traders or partnerships, limited companies provide limited liability protection, meaning the personal assets of directors and shareholders are protected in case of business debts or legal issues.",
      descriptiontwo:
        "Company formation is also necessary for businesses that wish to access certain tax benefits, raise investment, or appear more professional to clients and stakeholders. Directors of limited companies are legally responsible for filing annual accounts, Corporation Tax returns, and other statutory documents with Companies House. Even small businesses looking to expand or hire employees often choose limited company formation for better financial structure, credibility, and long-term growth opportunities.",
      img: companyfoormation.src,
    },
    getstarted: {
      heading: "Let's Get Started",
      descriptionone:
        "With Outsource Accounting, setting up a limited company in the UK is quick, reliable, and rewarding. Take the first step toward success today by starting your UK company registration journey with us",
      descriptiontwo: "",
    },
    statics: {
      heading: "How to Register a Company in the UK",
      description: "Follow these simple steps to get started with us:",
      imgone: statisticsone.src,
      imgtwo: statisticstwo.src,
      imgthree: statisticsthree.src,
      data: [
        {
          key: 1,
          img: tickCircle.src,
          heading: "Choose Your Company Name:",
          description:
            "Use our name-checking tool to ensure availability and compliance.",
        },
        {
          key: 2,
          img: tickCircle.src,
          heading: "Submit Details:",
          description:
            "Provide your director information, SIC code, and registered office address.",
        },
        {
          key: 3,
          img: tickCircle.src,
          heading: "Receive Your Documents",
          description:
            "We’ll deliver your Certificate of Incorporation and other official documents straight to your inbox.",
        },
      ],
    },
  },
  // ========== SERVICE 7: Corporation Tax Services (CT600) ==========
  "Corporation-Tax": {
    title: "Corporation Tax ",
    titleHighlight: "Services (CT600)",
    subtitle: "",
    description: [
      "Managing corporation tax is a crucial yet complex aspect of running a UK-based business. At Affotax, we provide expert Corporation Tax Return (CT600) filing services designed to keep your company compliant with HMRC regulations while maximising tax-saving opportunities.",
      "",
    ],
    buttonText: "Get Started Now",
    image: HSEVEN.src,
    bgImage: HeroBackgroundImg.src,
    whatYouGet: {
      heading: "What You Will Get",
      whatYouGetcards: [
        {
          key: 1,
          img: parkoutline.src,
          title: "Comprehensive Online Accounting Services for UK Businesses",
          description:
            "Outsource Accounting offers a suite of core financial and accounting services designed to support businesses of all sizes. These include monthly bookkeeping, accounts preparation, corporation tax filing, self-assessment tax returns, payroll management, and VAT return submissions.",
        },
        {
          key: 2,
          img: affordableCleanEnergy.src,
          title: "Affordable Pricing with Transparent, Flat-Rate Fees",
          description:
            "One of the standout benefits highlighted on the website is clear and affordable pricing. Outsource Accounting emphasises flat-rate pricing with no hidden charges — meaning you know what you'll pay from the start, whether you're a small start-up or an established company.",
        },
        {
          key: 3,
          img: userexpert.src,
          title: "Expert Support from Professional UK Accountants",
          description:
            "Instead of automated bots or generic financial software, the services are delivered by a real team of expert accountants who provide personalised one-on-one support. This means you get direct access to professionals who can explain complex financial requirements.",
        },
        {
          key: 4,
          img: addbusiness.src,
          title: "Insightful Financial Reporting Tailored to Your Business",
          description:
            "Outsource Accounting doesn't just manage numbers — they provide insightful financial reporting that goes beyond basic bookkeeping. These reports are designed to give you a clearer picture of your company's financial health, helping you understand profitability, cash flow trends, and tax obligations.",
        },
        {
          key: 5,
          img: arttrack.src,
          title: "Supportive Mission and Proven Track Record",
          description:
            "According to the website's 'About Us' section, the company started with a simple mission: to make professional accounting affordable and accessible to all UK businesses. With over two decades of experience and more than 1,000 clients served, Outsource Accounting positions itself as a reliable and trustworthy partner committed to helping businesses.",
        },
        {
          key: 6,
          img: housecompliance.src,
          title: "Tax Filing, HMRC and Companies House Compliance",
          description:
            "A core part of the service is handling all regulatory filings required in the UK. This includes Corporation Tax returns, VAT return submissions, annual accounts filings with Companies House, and Self-Assessment tax returns for business owners. Outsource Accounting liaises with HM Revenue & Customs (HMRC) and Companies House on your behalf.",
        },
      ],
    },
    whyChooseUs: {
      heading: "What is Corporation Tax (CT600)?",
      img: corporatetax.src,
      whyChooseUscards: [
        {
          key: 1,
          img: tickCircle.src,
          title: "",
          description:
            "Corporation Tax is a mandatory tax levied on the profits of UK-registered limited companies. Every active company must file a Corporation Tax Return (CT600) with HMRC within 12 months of the financial year-end. However, tax payments are due within 9 months and 1 day after the accounting period ends.",
        },
        {
          key: 2,
          img: tickCircle.src,
          title: "",
          description:
            "Failure to meet these deadlines can lead to significant penalties and interest charges. Affotax ensures your tax returns are prepared accurately and submitted on time, preventing unnecessary financial burdens.",
        },
      ],
    },
    ServiceData: {
      heading: "Why Choose Affotax for Corporation Tax Filing?",
      highlightHeading: "",
      ServiceData: [
        {
          index: 1,
          imgSrc: gettingtoknow.src,
          title: "Experienced Accountants",
          description:
            "Our team brings years of expertise in UK tax regulations.",
        },
        {
          index: 2,
          imgSrc: prepareaccount.src,
          title: "Tailored Services",
          description:
            "We use Nomisma cloud software for real-time tax tracking",
        },
        {
          index: 3,
          imgSrc: reviewexplain.src,
          title: "Personalised Tax Strategies",
          description:
            "Tailored advice to help businesses reduce tax liability legally.",
        },
        {
          index: 4,
          imgSrc: everytime.src,
          title: "Proactive Tax Planning",
          description:
            "We don’t just file your CT600—we help you plan your tax strategy",
        },
      ],
    },
    WhatData: {
      heading: "What Is Corporation Tax?",
      descriptionone:
        "Corporation Tax is a tax levied on the profits of UK-based companies and organisations. Unlike income tax, which applies to individuals, Corporation Tax is paid by limited companies, foreign companies with UK branches, and certain organisations on the profits they generate from trading, investments, or selling assets. The tax applies to all profits, including revenue from business activities, investments, and capital gains.",
      descriptiontwo:
        "Corporation Tax is an essential part of running a limited company in the UK. Accurate calculation and timely payment of Corporation Tax ensure compliance with HMRC regulations and avoid penalties or interest for late payments. Companies must prepare annual accounts and compute taxable profits based on revenue minus allowable expenses, reliefs, and deductions. Filing a Company Tax Return with HMRC is a legal requirement, and the process integrates with other accounting practices, such as bookkeeping and payroll, to maintain accurate financial records. Proper Corporation Tax management helps businesses plan their finances, reinvest profits, and make informed decisions for growth.",
      img: waa.src,
    },
    WhoData: {
      heading: "Who Needs Corporation Tax in the UK?",
      descriptionone:
        "In the UK, all limited companies must pay Corporation Tax on their profits, regardless of whether they are actively trading or dormant. This includes private limited companies (Ltd), public limited companies (PLC), and certain clubs, associations, or organisations that generate taxable profits. Companies with UK branches but registered abroad may also be liable for UK Corporation Tax on profits earned within the country.",
      descriptiontwo:
        "Sole traders and partnerships do not pay Corporation Tax; instead, they pay Income Tax on their business profits through Self Assessment. However, limited company directors must ensure that their company’s taxable profits are calculated accurately, Corporation Tax returns are filed on time, and payments are made by HMRC deadlines. Proper management of Corporation Tax is crucial for compliance, avoiding fines, and maintaining a transparent financial record for investors, lenders, and regulatory authorities.",
      img: corporation.src,
    },
    getstarted: {
      heading: "Let's Get Started",
      descriptionone:
        "Let Us handle your CT600 filing and corporation tax matters with precision and expertise. Contact us today to discuss how we can help your business stay compliant and maximise tax savings.",
      descriptiontwo: "",
    },
    statics: {
      heading: "Penalties for Late CT600 Filing",
      description:
        "Missing the corporation tax filing deadline can result in escalating penalties:",
      imgone: statisticsone.src,
      imgtwo: statisticstwo.src,
      imgthree: statisticsthree.src,
      data: [
        {
          key: 1,
          img: tickCircle.src,
          heading: "1 day late: ",
          description: "£100 fine",
        },
        {
          key: 2,
          img: tickCircle.src,
          heading: "3 months late: ",
          description: "Additional £100 fine",
        },
        {
          key: 3,
          img: tickCircle.src,
          heading: "6 months late: ",
          description:
            "HMRC estimates your bill and imposes a 10% penalty on the estimate",
        },
        {
          key: 4,
          img: tickCircle.src,
          heading: "12 months late: ",
          description: "An extra 10% penalty on the estimated bill",
        },
      ],
    },
  },
};
